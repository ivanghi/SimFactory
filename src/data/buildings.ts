export interface BuildingDef {
  id: string;
  name: string;
  category: 'extractor' | 'processor' | 'power' | 'storage';
  cost: { resource: string; amount: number }[];
  footprint: { width: number; height: number };
  powerDraw: number;
  recipeId: string;
  placementRule: string;
  powerOutput?: number;
}

export const buildings: BuildingDef[] = [
  // Extractors
  {
    id: 'iron-miner',
    name: 'Iron Miner',
    category: 'extractor',
    cost: [{ resource: 'iron-ore', amount: 10 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 1,
    recipeId: 'iron-ore',
    placementRule: 'resource'
  },
  {
    id: 'copper-miner',
    name: 'Copper Miner',
    category: 'extractor',
    cost: [{ resource: 'iron-ore', amount: 10 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 1,
    recipeId: 'copper-ore',
    placementRule: 'resource'
  },
  {
    id: 'coal-miner',
    name: 'Coal Miner',
    category: 'extractor',
    cost: [{ resource: 'iron-ore', amount: 10 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 1,
    recipeId: 'coal',
    placementRule: 'resource'
  },
  {
    id: 'pumpjack',
    name: 'Pumpjack',
    category: 'extractor',
    cost: [{ resource: 'iron-ingot', amount: 15 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 1,
    recipeId: 'crude-oil',
    placementRule: 'resource'
  },
  {
    id: 'water-pump',
    name: 'Water Pump',
    category: 'extractor',
    cost: [{ resource: 'iron-ingot', amount: 10 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 1,
    recipeId: 'water',
    placementRule: 'resource'
  },

  // Processors
  {
    id: 'smelter',
    name: 'Smelter',
    category: 'processor',
    cost: [{ resource: 'iron-ore', amount: 10 }, { resource: 'copper-ore', amount: 10 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 2,
    recipeId: 'smelter',
    placementRule: 'factory'
  },
  {
    id: 'copper-smelter',
    name: 'Copper Smelter',
    category: 'processor',
    cost: [{ resource: 'copper-ore', amount: 10 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 2,
    recipeId: 'copper-smelter',
    placementRule: 'factory'
  },
  {
    id: 'refinery',
    name: 'Refinery',
    category: 'processor',
    cost: [{ resource: 'iron-ingot', amount: 15 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 3,
    recipeId: 'refinery',
    placementRule: 'factory'
  },
  {
    id: 'chemical-plant',
    name: 'Chemical Plant',
    category: 'processor',
    cost: [{ resource: 'iron-ingot', amount: 20 }, { resource: 'copper-ingot', amount: 20 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 3,
    recipeId: 'chemical-plant',
    placementRule: 'factory'
  },
  {
    id: 'gear-assembler',
    name: 'Gear Assembler',
    category: 'processor',
    cost: [{ resource: 'iron-ingot', amount: 10 }, { resource: 'copper-ingot', amount: 20 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 2,
    recipeId: 'gear-assembler',
    placementRule: 'factory'
  },
  {
    id: 'circuit-assembler',
    name: 'Circuit Assembler',
    category: 'processor',
    cost: [{ resource: 'copper-ingot', amount: 10 }, { resource: 'plastic', amount: 5 }, { resource: 'iron-ingot', amount: 10 }, { resource: 'gears', amount: 10 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 3,
    recipeId: 'circuit-assembler',
    placementRule: 'factory'
  },
  {
    id: 'lab',
    name: 'Lab',
    category: 'processor',
    cost: [{ resource: 'circuits', amount: 10 }, { resource: 'gears', amount: 20 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 4,
    recipeId: 'lab',
    placementRule: 'factory'
  },

  // Power/storage
  {
    id: 'solar-panel',
    name: 'Solar Panel',
    category: 'power',
    cost: [{ resource: 'iron-ingot', amount: 5 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 0,
    powerOutput: 2,
    recipeId: 'solar-panel',
    placementRule: 'power'
  },
  {
    id: 'coal-generator',
    name: 'Coal Generator',
    category: 'power',
    cost: [{ resource: 'iron-ingot', amount: 10 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 0,
    powerOutput: 6,
    recipeId: 'coal-generator',
    placementRule: 'power'
  },
  {
    id: 'warehouse',
    name: 'Warehouse',
    category: 'storage',
    cost: [{ resource: 'iron-ingot', amount: 15 }],
    footprint: { width: 1, height: 1 },
    powerDraw: 0,
    recipeId: 'warehouse',
    placementRule: 'storage'
  },
];

export function getBuildingDef(id: string): BuildingDef | undefined {
  return buildings.find((building) => building.id === id);
}